...
// dr/[slug].js
var BACKEND = "https://vyasa-os-backend.onrender.com";
...
function buildJsonLd(d, pageUrl) {
  ...
  const ld = {
    ...
    name: `Dr. ${d.name}`,
    url: pageUrl,
    medicalSpecialty: d.specialty || void 0,
    description: d.bio ? String(d.bio).slice(0, 300) : void 0,
    image: d.profilePhotoUrl || void 0,
    priceRange: d.consultationFee ? `\u20B9${d.consultationFee}` : void 0
  };
  if (clinic?.address || clinic?.city) {
    ld.address = {
      "@type": "PostalAddress",
      streetAddress: clinic.address || void 0,
      addressLocality: clinic.city || void 0,
      addressRegion: clinic.state || void 0,
      postalCode: clinic.pincode || void 0,
      addressCountry: "IN"
    };
  }
...
function injectMeta(html, d, pageUrl) {
  ...
  html = html.replace(/<title>[\s\S]*?<\/title>/, `<title>${title}</title>`).replace(/(<meta name="description" content=")[^"]*(")/, `$1${desc}$2`).replace(/(<meta property="og:title" content=")[^"]*(")/, `$1${title}$2`).replace(/(<meta property="og:description" content=")[^"]*(")/, `$1${desc}$2`).replace(/(<meta property="og:url" content=")[^"]*(")/, `$1${esc(pageUrl)}$2`).replace(/(<meta property="og:type" content=")[^"]*(")/, "$1profile$2").replace(/(<meta name="twitter:title" content=")[^"]*(")/, `$1${title}$2`).replace(/(<meta name="twitter:description" content=")[^"]*(")/, `$1${desc}$2`);
  const extra = `<link rel="canonical" href="${esc(pageUrl)}" />
...
// sitemap.xml.js
var BACKEND2 = "https://vyasa-os-backend.onrender.com";
async function onRequestGet2() {
  try {
    const r = await fetch(`${BACKEND2}/public/sitemap.xml`, {
...
// ../.wrangler/tmp/pages-7PcUvf/functionsRoutes-0.788596372002067.mjs
var routes = [
  {
    routePath: "/dr/:slug",
    mountPath: "/dr",
